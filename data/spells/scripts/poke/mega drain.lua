...
function onCastSpell(cid, var)
	...
	if getPlayerStorageValue(cid, 5) >= 1 then
		...
		local levels = huah
		doTargetCombatHealth(getCreatureTarget(cid), cid, COMBAT_PHYSICALDAMAGE, -(math.random((getPlayerLevel(getCreatureTarget(cid))*3),(getPlayerLevel(getCreatureTarget(cid))*5))), -((math.random((getPlayerLevel(getCreatureTarget(cid))*3),(getPlayerLevel(getCreatureTarget(cid))*5))+10)), 3)
		return true
		end
	end
