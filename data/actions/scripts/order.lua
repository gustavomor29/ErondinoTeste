...
if not isInArray({"Alakazam", "Tentacruel" , "Gengar" , "Haunter" , "Shiny Tentacruel" , "Shiny Gengar" , "Slowking" , "Shiny Alakazam", "Ditto"}, getCreatureName(getCreatureSummons(cid)[1])) then
...
if isMonster(item2.uid) == TRUE and getCreatureName(getCreatureSummons(cid)[1]) == "Ditto" then
   if getPlayerStorageValue(item2.uid, 202) >= 1 then 
     ...
     doCreatureSay(cid, "Ditto, transformation ".. getCreatureName(item2.uid) .."!", 1)
	 ...
	 doItemSetAttribute(getPlayerSlotItem(cid, 8).uid, "ditto", "trasformado")
	 doUpdateMoves(cid)
return TRUE
end
...
doRemoveCreature(getCreatureSummons(cid)[1])
pk = doSummonCreature(getItemAttribute(item.uid, "poke"), mynewpos)
doConvinceCreature(cid, pk)
doCreatureSetLookDir(getCreatureSummons(cid)[1], 2)
registerCreatureEvent(pk, "DiePoke")
registerCreatureEvent(pk, "Exp")
...
exhaustion.set(cid, etudao, 4)
	...
	local item = getPlayerSlotItem(cid, 8)
	doCreatureSay(cid, "" .. getItemAttribute(item.uid, "poke") ..", let me get down!", 1)
...
if getItemAttribute(getPlayerSlotItem(cid, 8).uid, 'ditto') == "trasformado" then 
...
doItemSetAttribute(getPlayerSlotItem(cid, 8).uid, "ditto", "noTrans")
doPlayerSendCancel(cid, "12//,hide")
else
doPlayerSendCancel(cid, "Seu Ditto Nao esta transformado.")
end
return TRUE
end
