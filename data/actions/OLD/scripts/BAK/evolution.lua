...
["Elekid"] = {level = 35, cons = 150, vida = 2500},
["Electabuzz"] = {level = 70, cons = 150, vida = 2500},
["Shiny Electabuzz"] = {level = 70, cons = 150, vida = 2500},
...
if getCreatureName(item2.uid) == "Elekid" then
        if item.itemid == 2279 then
                if getPlayerLevel(cid) < 70 then
                return doPlayerSendCancel(cid, "Voce precisa de level 70 para evoluir esta pokemon.")
                end
				local buzz = math.random(1, 5)
				if (buzz <= 1) then
                local pokeball = getPlayerSlotItem(cid, 8)
                doItemSetAttribute(pokeball.uid, "poke", "This is Shiny Electabuzz's pokeball. HP = [9500/9500]")
                doItemSetAttribute(pokeball.uid, "description", "Contains a Shiny Electabuzz.")
                doPlayerSendTextMessage(cid, 27, "Parabens! Seu "..getCreatureName(item2.uid).." Evoluiu num Shiny Electabuzz!")
                doSendMagicEffect(topos, 18)
                doTransformItem(getPlayerSlotItem(cid, 7).uid, 2511)
                doSendMagicEffect(getThingPos(cid), 173)
                local oldpos = getThingPos(item2.uid)
                doRemoveCreature(item2.uid)
                doSummonMonster(cid, "Shiny Electabuzz")
                local pk = getCreatureSummons(cid)[1]
                registerCreatureEvent(pk, "DiePoke")
                registerCreatureEvent(pk, "Exp")
                doTeleportThing(pk, oldpos, false)
				...
				local pokeball = getPlayerSlotItem(cid, 8)
                doItemSetAttribute(pokeball.uid, "poke", "This is Electabuzz's pokeball. HP = [6500/6500]")
                doItemSetAttribute(pokeball.uid, "description", "Contains a Electabuzz.")
                doPlayerSendTextMessage(cid, 27, "Congratulations! Your "..getCreatureName(item2.uid).." evolved into a Electabuzz!")
                doSendMagicEffect(topos, 18)
                doTransformItem(getPlayerSlotItem(cid, 7).uid, 2511)
                doSendMagicEffect(getThingPos(cid), 173)
                local oldpos = getThingPos(item2.uid)
                doRemoveCreature(item2.uid)
                doSummonMonster(cid, "Electabuzz")
                local pk = getCreatureSummons(cid)[1]
                registerCreatureEvent(pk, "DiePoke")
                registerCreatureEvent(pk, "Exp")
                doTeleportThing(pk, oldpos, false)
