...
storage = 50003 --storage id
item = 11999 -- feather, ou outro item q ele pede pra buscar
quantiitem = 2 -- quantidade de item q precisa pegar
premio1 = 7885 -- item do premio no caso dinheiro
	...
	if getDistanceToCreature(cid) > 4 then
	  ...
	  if talkState[talkUser] ~= 3 then
		...
		if (msgcontains(msg, 'hi'))  then
				...
				fala = "olá! eu preciso urgente de 2 luck medalion para um projeto meu... pode me ajudar?"
		...
		quest1 = getPlayerStorageValue(cid,storage)
			...
			if quest1 ==1 then
				selfSay('Bom! Então vc trouxe minhas luck medalion? Deixe-me ve-las.',cid)
				if doPlayerRemoveItem(cid, item, quantiitem) == true then
					...
					selfSay('Voce não trouxe minhas luck medalions... Volte quando tivelas.',cid)
					talkState[talkUser] = 0
				end
				
			else
				selfSay('Me traga 2 luck medalion  para um projeto meu, Se voce me trazer te recompensarei com 1 box 2 e 3000 de experiencia!',cid)
