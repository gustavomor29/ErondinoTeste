...
storage = 50002 --storage id
item = 8864 -- feather, ou outro item q ele pede pra buscar
quantiitem = 75 -- quantidade de item q precisa pegar
premio1 = 2280 -- item do premio no caso dinheiro
	...
	if getDistanceToCreature(cid) > 4 then
	  ...
	  if talkState[talkUser] ~= 3 then
		...
		if (msgcontains(msg, 'hi'))  then
				...
				fala = "olá! eu preciso urgente de 75 stone orb para um projeto meu... pode me ajudar?"
		...
		quest1 = getPlayerStorageValue(cid,storage)
			...
			if quest1 ==1 then
				selfSay('Bom! Então vc trouxe minhas stone orb? Deixe-me ve-las.',cid)
				if doPlayerRemoveItem(cid, item, quantiitem) == true then
					...
					selfSay('Voce não trouxe minhas stone orb... Volte quando tivelas.',cid)
					talkState[talkUser] = 0
				end
				
			else
				selfSay('Me traga 75 stone orb  para um projeto meu, Se voce me trazer te recompensarei com 1 rock stone e 5000 de experiencia!',cid)
