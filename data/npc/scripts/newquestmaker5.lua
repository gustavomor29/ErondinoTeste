...
premio1 = 12160 -- item do premio no caso dinheiro
	...
	if getDistanceToCreature(cid) > 4 then
	  ...
	  if talkState[talkUser] ~= 3 then
		...
		if (msgcontains(msg, 'hi'))  then
				...
				fala = "Iai irmao, ta afim de vim para o mundo do crime parceiro?"
		...
		quest1 = getPlayerStorageValue(cid,storage)
			if quest1 == 2 then
				selfSay('Seu trouxa vc é da policia sai fora daqui!',cid)
			else
				
			if quest1 ==1 then
				selfSay('Bom! Vc tem certeza?.',cid)
				if doPlayerRemoveItem(cid, item, quantiitem) == true then
					selfSay('Isso ai doido seja bem vindo!',cid)
				...
				selfSay('Quer entrar para a equipe rocket?!',cid)
