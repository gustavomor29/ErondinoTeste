...
if((msgcontains(msg, 'hi') or msgcontains(msg, 'heal') or msgcontains(msg, 'help')) and (getDistanceToCreature(cid) <= 3)) then
	...
	local maxh = tonumber(getItemAttribute(feet.uid, "maxhealth"))
	doItemSetAttribute(feet.uid, "hp", maxh)
    doItemSetAttribute(feet.uid, "maxhealth", maxh)
	...
	doTransformItem(feet.uid, 11627)
		...
		local bp = getPlayerSlotItem(cid, CONST_SLOT_BACKPACK)
			...
			local maxh = tonumber(getItemAttribute(uid, "maxhealth"))
			doItemSetAttribute(uid, "hp", maxh)
            doItemSetAttribute(uid, "maxhealth", maxh)
