	...
	if getDistanceToCreature(cid) > 4 then
	  ...
	  if talkState[talkUser] ~= 3 then
		...
		if (msgcontains(msg, 'hi'))  then
				...
				fala = "olá! eu preciso urgente de 30 zubat wings para um projeto meu... pode me ajudar?"
		...
		quest1 = getPlayerStorageValue(cid,storage)
			...
			if quest1 ==1 then
				selfSay('Bom! Então vc trouxe as zubat wings? Deixe-me ve-lo.',cid)
				if doPlayerRemoveItem(cid, item, quantiitem) == true then
					...
					selfSay('Voce não trouxe mminhas zubat wings... Volte quando tiver.',cid)
					talkState[talkUser] = 0
				end
				
			else
				selfSay('Me traga 30 zubat wings para um projeto meu, Se voce me trazer te recompensarei com 1 venom stone e 5000 de experiencia!',cid)
